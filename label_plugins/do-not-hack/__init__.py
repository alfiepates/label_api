...
ARGUMENTS = {
  'owner': {'help': 'hacklab username of owner', 'type': str, 'default': 'INVALID-DNH'},
  'date_in': {'help': 'Date of DNH issue', 'type': datetime.date, 'default': datetime.date.today()},
  'date_out': {'help': 'Date item will be removed', 'type': datetime.date, 'default': (datetime.date.today() + datetime.timedelta(days=14))},
}
...
def create_label(owner="", date_in="", date_out=""):
    if not owner: owner = ARGUMENTS['owner']['default']
    if not date_in: date_in = ARGUMENTS['date_in']['default']
    if not date_out: date_out = ARGUMENTS['date_out']['default']
    ...
    font = fit_text(owner, LABEL_SIZE[0], font_path, ifs)
    draw.text((x_pos, y_pos), owner, font=font, fill=0)
    ## LAST NAME
